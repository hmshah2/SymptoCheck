...
def find_sym(sym,sev) -> dict:
    ...
    query_2 = text("SELECT t.Medicine AS `count_Treatment_Method` FROM Has_Symptom s JOIN Has_Treatment t ON s.IllnessName = t.IllnessName WHERE s.SymptomName = '{}' GROUP BY t.Medicine ORDER BY `count_Treatment_Method` DESC
LIMIT 15;
")
    return ill_list
