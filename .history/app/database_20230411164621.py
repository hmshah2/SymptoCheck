...
def find_sym(sym,sev) -> dict:
    ...
    if len(results) != 0:
        ...
        return ill_dict
    query_2 = text("SELECT t.Medicine AS `count_Treatment_Method` FROM Has_Treatment t JOIN Has_Symptom s ON t.IllnessName = s.IllnessName WHERE s.SymptomName = '{}' GROUP BY t.Medicine ORDER BY COUNT(t.Medicine) DESC LIMIT 15;".format(sym))
