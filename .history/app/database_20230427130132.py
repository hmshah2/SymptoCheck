...
def sym_rank() -> str:
    conn = db.connect()
    query = text("SELECT SymptomName FROM Symptoms WHERE SymptomRank = 1 LIMIT 1;")
...
def admin_update_record(sym:str,ill:str,sev:int) -> str:
    ...
    conn.execute(text("START TRANSCATION;"))

    query = text("""
            
            DECLARE @ill AS VARCHAR(255)= '{}'
            DECLARE @sym AS VARCHAR(255)= '{}'
            DECLARE @sev AS INTEGER = {}
