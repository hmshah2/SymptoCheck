...
def find_sym(sym,sev) -> list:
    ...
    if type(sym) != NoneType:
        ...
        #results = conn.execute(text("SELECT * FROM Has_Symptom LIMIT 2;").fetchall()
    conn.close()
    ill_list = []
    if indicator == 1:
        ill_list = results
...
def ill_sta(ill) -> dict:
    ill_sta = {}
    conn = db.connect()
    query_1 = "SELECT Medicine FROM Has_Treatment WHERE IllnessName LIKE {} GROUP BY Medicine ORDER BY COUNT(Medicine) DESC LIMIT 10;".format(ill)
    cur_1 = conn.execute(query_1)
    results_1 = cur_1.fetchall()
    ill_sta['treatment'] = results_1
    query_2 = "SELECT SymptomName FROM Has_Symptom WHERE IllnessName LIKE {} GROUP BY SymptomName ORDER BY COUNT(SymptomName) DESC LIMIT 10;".format(ill)
...
def find_user(user:str,pwd:str) -> int:
    ...
    if type(user) != NoneType:
        ...
        query = 'SELECT password FROM User WHERE user_name = {}'.format(user)
...
def find_record(user:str) -> list:
    conn = db.connect()
    query = "SELECT * FROM Records WHERE user_id = {} ORDER BY checkin_date DESC LIMIT 10;"
...
def update_record(user:str,sym:str,age:int,sex:str,sev:int) -> None:
    conn = db.connect()
    query_check = 'SELECT * FROM Records WHERE user_id = {} AND age = {} AND sex = {} AND trackable_type = "Symptom" AND trackable_name = {} and trackable_value = {} LIMIT 1;'.format(user,age,sex,sym,sev)
