...
def admin_update_record(sym:str,ill:str,sev:int) -> str:
    ...
    result = conn.execute(text("SELECT COUNT(*) FROM Has_Symptom WHERE IllnessName = 'anxiety' AND SymptomName = 'Anxiety' AND Severity = 3;")).fetchall()
    print(result)
    query = text("""
            START TRANSACTION;
   
            SELECT COUNT(IllnessName) INTO @find_cnt FROM Has_Symptom WHERE IllnessName = :ill AND SymptomName = :sym AND Severity = :sev;

            IF @find_cnt > 0 THEN
                INSERT INTO Has_Symptom (IllnessName, SymptomName, Severity) VALUES (:ill, :sym, :sev);
            ELSE
                INSERT INTO Symptoms (SymptomName, Severity) VALUES (:sym, :sev);
                INSERT INTO Illness (IllnessName) VALUES (:ill);
            END IF;
