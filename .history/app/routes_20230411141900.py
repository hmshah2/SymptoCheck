...
def illsta():
    ...
    if ill_content == None:
        return render_template("search_treatments.html")
    if (request.form['action'] == "sex"):
        search_Tre = db_helper.ill_sta(ill_content)
        return render_template("search_treatments.html",results= search_Tre)
    elif request.form['action'] == "ill":
        search_Sex = db_helper.sex_sta(ill_content)
        return render_template("search_treatments.html",results= search_Sex)

@app.route('/illpage/sexSta',methods = ['GET','POST'])
def sexsta():
    ill_content = request.form['ill-input']
    if ill_content == None:
        return render_template("search_treatments.html")
    search_sex = db_helper.ill_sta(ill_content)
    return render_template("search_treatments.html",sex_results= search_sex)