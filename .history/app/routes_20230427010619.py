...
def update_admin():
    ...
    upd_result = db_helper.admin_update_record(sym,ill,sev)
    return upd_result
...
def search():
    ...
    return_result = [search_Sym,rank_first]
    return render_template("index.html",results = [search_Sym,rank_first])
...
def illsta():
    ...
    if (request.form['action'] == "search for treatments"):
        ...
        top_sym = search_Tre['symptom'][0]
        top_tre = search_Tre['treatment'][0]
        db_helper.update_sym_record(user_id,top_sym,ill_content,top_tre,1)
