...
def admin_update_record(sym:str,ill:str,sev:int) -> str:
    ...
    query = text("""
        ...
        BEGIN TRANSACTION;
        IF %(sev)s IS NULL THEN
            SET %(sev)s = 1;
        END IF;
        IF %(sym)s IS NOT NULL AND %(ill)s IS NOT NULL THEN
            INSERT INTO Has_Symptom(IllnessName,SymptomName,Severity) VALUES(%(ill)s,%(sym)s,%(sev)s);
        ELSEIF %(sym)s IS NOT NULL THEN
            INSERT INTO Symptoms(SymptomName,Severity) VALUES(%(sym)s,%(sev)s);
        ELSEIF %(ill)s IS NOT NULL THEN
            INSERT INTO Illness(IllnessName) VALUES(%(ill)s);
        END IF;
        COMMIT;
        """)
