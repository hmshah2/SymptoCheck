...
def find_sym(sym,sev) -> dict:
    ...
    ill_list = {}
    if indicator == 1:
        ill_list['illness'] = results
    else:
        ill_list['illness'] = ['no data']
        ill_list['treatment'] = ['no data']
    query_2 = text("SELECT t.Medicine AS `count_Treatment_Method` FROM Has_Symptom s JOIN Has_Treatment t ON s.IllnessName = t.IllnessName WHERE s.SymptomName = '{}' GROUP BY t.Medicine ORDER BY COUNT(t.Medicine) DESC LIMIT 15;".format(sym))
    results_2 = conn.execute(query_2).fetchall()
    ill_list['treatment'] = results_2
