...
def sex_sta(ill) -> list:
    text("SELECT f.Illness, f.Num_Female, m.Num_Male
FROM
  (SELECT trackable_name AS Illness, COUNT(user_id) as Num_Female
   FROM Records
   WHERE trackable_name = '{}' AND sex = 'female'
   GROUP BY Illness) f
JOIN
  (SELECT trackable_name AS Illness, COUNT(user_id) as Num_Male
   FROM Records
   WHERE trackable_name = '{}' AND sex = 'male'
   GROUP BY Illness) 
ON f.Illness = m.Illness
ORDER BY f.Num_Female DESC, m.Num_Male DESC
LIMIT 15;").format(ill,ill)
