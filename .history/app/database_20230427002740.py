...
def admin_update_record(sym:str,ill:str,sev:int) -> str:
    ...
    query = text("""
            ...
            DECLARE ill VARCHAR(255) DEFAULT '{}';
            DECLARE sym VARCHAR(255) DEFAULT '{}';
            DECLARE sev INT DEFAULT {};

            IF ill IS NOT NULL AND sym IS NOT NULL THEN
                SELECT COUNT(*) INTO @find FROM Has_Symptom WHERE IllnessName = ill AND SymptomName = sym AND Severity = sev;
                IF @find = 0 THEN
